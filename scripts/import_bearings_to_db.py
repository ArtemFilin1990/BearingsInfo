...
def import_all_data(conn):
    ...
    try:
        ...
        combined = master.merge(
            dimensions[['Designation', 'Dynamic_Load_C_kN', 'Static_Load_C0_kN', 
                       'Limiting_Speed_rpm', 'Reference_Speed_rpm']],
            left_on='ISO',
        ...
        for _, row in combined.iterrows():
            try:
                ...
                """, (
                    row['GOST'], row['ISO'], row['SKF'], row['FAG'],
                    row['NSK'], row['NTN'], row['KOYO'], row['Type'],
                    row['d'], row['D'], row['B'], row['r_min'], row['Weight_kg'],
                    row.get('Dynamic_Load_C_kN'), row.get('Static_Load_C0_kN'),
                    row.get('Limiting_Speed_rpm'), row.get('Reference_Speed_rpm'),
