...
def _load_schema_payload(path: Path) -> Dict:
    raw_text = path.read_text(encoding="utf-8")
    if yaml is not None:
        return yaml.safe_load(raw_text)  # type: ignore[arg-type]
    try:
        return json.loads(raw_text)
    except json.JSONDecodeError as exc:  # pragma: no cover - defensive
        raise RuntimeError(
            f"{path} is not JSON-compatible and PyYAML is unavailable: {exc}"
        ) from exc


def _load_schema_file(path: Path) -> List[TableSchema]:
    payload = _load_schema_payload(path)
