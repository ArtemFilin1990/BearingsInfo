...
def temp_workspace(tmp_path):
    ...
    content = content.replace(
        'file: "out/processed_registry.json"', f'file: "{workspace["out"]}/processed_registry.json"'
    )
...
def processor(temp_workspace):
    ...
    logger = LoggerSetup.setup(temp_workspace["logs"] / "app.log", "json", "INFO")
...
def test_dimension_conflict_detection(temp_workspace, processor):
    ...
    data = pd.DataFrame(
        [
            {"Артикул": "6205", "Бренд": "SKF", "D": 52, "d": 25, "H": 15},
            {"Артикул": "6205", "Бренд": "SKF", "D": 54, "d": 25, "H": 15},  # Different dimension!
        ]
    )
...
def test_json_file_processing(temp_workspace, processor):
    ...
    # Create JSON file
    data = [{"Артикул": "6207", "Бренд": "FAG", "D": 72, "d": 35, "H": 17}]
