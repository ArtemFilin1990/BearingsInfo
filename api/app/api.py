...
from .export_utils import SearchResultsExporter, AnalogsExporter
...
async def autocomplete(
    ...
    types: Optional[List[str]] = Query(None, description="Типы: term, bearing_code, brand, series")
):
    """
...
async def search(
    ...
    user_id: Optional[str] = Query(None, description="Токен пользователя (опционально)"),
...
async def get_similar_documents(
    document_id: str,
    limit: int = Query(5, ge=1, le=20, description="Количество похожих документов")
):
    """
...
async def get_popular_queries(
    limit: int = Query(10, ge=1, le=50, description="Количество запросов")
):
    """
...
async def get_search_analytics(
    period: str = Query("7d", regex="^(1d|7d|30d)$", description="Период: 1d, 7d, 30d")
):
    """
...
async def export_batch(
    queries: List[str],
    export_format: str = Query("xlsx", regex="^(json|xlsx)$", description="Формат: json, xlsx")
):
    """
    ...
    if export_format == "json":
        ...
        return StreamingResponse(
            excel_data,
            media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            headers={"Content-Disposition": f"attachment; filename=batch_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"}
        )
