...
class KnowledgeBaseBuilder:
    ...
    def extract_code_structures(self, content: str, file_type: str) -> list[str]:
        ...
        if file_type == "Python":
            ...
            try:
                ...
                structures.append(
                    f"JSON structure with {len(data)} top-level keys"
    ...
    def generate_inventory_table(self) -> str:
        ...
        lines.extend(["", f"**Итого файлов:** {len(self.file_inventory)}", ""])
    ...
    def generate_glossary(self) -> str:
        ...
        for term in sorted(self.terms_glossary.keys()):
            ...
            lines.append(f"| {term} | {first_def[:100]}... | {source_count} источник(ов) |")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Knowledge Base Builder - построение базы знаний из репозитория")
    parser.add_argument("--repo", default=".", help="Путь к репозиторию (по умолчанию: текущая директория)")
    parser.add_argument(
        "--output", default="KNOWLEDGE_BASE.md", help="Имя выходного файла (по умолчанию: KNOWLEDGE_BASE.md)"
    )
